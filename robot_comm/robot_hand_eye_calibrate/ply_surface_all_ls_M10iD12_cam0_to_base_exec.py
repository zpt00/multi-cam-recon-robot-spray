...
PLY_PATH = r"C:\Users\23865\Desktop\realsense_2__\pcd2mesh\output_mesh\test.ply"

OUTPUT_DIR = r"C:\Users\Administrator\Desktop\open3D_learn\realsense_2__\output_trajectory"

# =========================================================
# 1.0 标定结果与最终 LS 坐标系设置
# =========================================================
# 说明：
# - PLY / 扫描轨迹默认认为是在 cam0 坐标系，单位为米；
# - 最终写入 FANUC LS 前，会使用 T_base_cam0 将轨迹点转换到机器人 base 坐标系；
# - 姿态 WPR 不是简单沿用 cam0 角度，而是先把表面法向和轨迹切线一起转换到 base，
#   再重新计算 W/P/R，确保工具轴仍然垂直于工件表面。
ENABLE_CAM0_TO_BASE_TRANSFORM_FOR_LS = True

# 手眼标定结果 YAML 文件路径。请改成你的 eye_to_hand_result.yaml 实际路径。
...
# ---------------------------------------------------------
# 模型单位设置
# 重要: mesh 坐标单位为米，参数按米设置
# 模型尺寸约 0.33×0.30×0.28m，步长设 0.01m (10mm) 约每面 900 条射线
# ---------------------------------------------------------

RASTER_STEP = 0.05

# 射线起点距离模型包围盒的外扩距离（米）
RAY_START_MARGIN = 0.005
...
# FANUC 程序基础参数
LS_PROG_PREFIX = "M10PATH"

# 多个面的轨迹汇总到一个 LS 文件时，固定输出这个文件名。
# 注意：这是磁盘文件名；LS 文件内部 /PROG 名称仍会经过 sanitize_fanuc_program_name()
# 处理，以适配 FANUC 程序名长度和字符限制。
LS_COMBINED_OUTPUT_FILENAME = "M10D12_EXEC.ls"

# =========================================================
# FANUC 喷涂工艺 LS 文件头格式
# =========================================================
# True：使用带 /APPL 和 PAINT_PROCESS 的喷涂工艺格式；M-10iD/12 普通机械臂默认 False。
LS_STRICT_PAINT_HEADER = False

# 汇总总程序在 LS 内部显示的程序名和 FILE_NAME。
# 按用户指定，必须是 TEST20250910WK2 + Process。
LS_COMBINED_PROGRAM_DISPLAY_NAME = "M10D12EXEC"

# FILE_NAME / PROG 行后面的工艺标记。
LS_PROGRAM_PROCESS_SUFFIX = ""
...
UTOOL_NUM = 1
CNT_VALUE = 100
SAFE_CONFIG = "N U T, 0,-1, 1"
CONFIG_STR = SAFE_CONFIG

# 是否在 LS 中写入 HOME 点。
# 为了降低“位置形态信息不一致 / 原点不可达”的概率，默认 False。
# 如果你已在机器人上示教了真实安全 HOME，再改为 True 并填写 LS_HOME_XYZWPR。
LS_USE_HOME_POINTS = False

# 机器人安全 HOME。注意：这里始终是机器人 base 坐标，不会再经过 cam0→base 标定转换。
# 格式：X, Y, Z, W, P, R；XYZ 单位为米，WPR 单位为度。
...
# 每个面开始喷涂前/结束喷涂后，沿表面外法向后退的安全距离（mm）。
# 工件约 300mm，退回 50~80mm 足够
LS_SAFE_RETRACT_DISTANCE_MM = 0.08   # 80mm = 0.08m（PLY单位是米）

# 是否在每个 face 内部的不同 segment 之间也后退到安全空间
LS_RETRACT_BETWEEN_SEGMENTS = True

# 每个面完成后是否回到原点/安全原点
LS_RETURN_HOME_AFTER_EACH_FACE = False

# combined 总程序中，每个面之间是否回原点/安全原点
LS_RETURN_HOME_BETWEEN_FACES = False
...
VISUALIZE_FINAL_LS_SPRAY_POINTS = False

# True：显示最终 LS 的接近点 / 后退点
VISUALIZE_FINAL_LS_APPROACH_RETREAT_POINTS = False

# True：显示最终 LS 的 HOME 点
VISUALIZE_FINAL_LS_HOME_POINTS = False

# True：在部分最终 LS 喷涂点上显示姿态坐标轴
VISUALIZE_FINAL_LS_POSE_AXES = False

# 每隔多少个喷涂点显示一个姿态坐标轴
FINAL_LS_POSE_AXIS_EVERY_N = 5

# 姿态坐标轴长度（米单位，模型约 0.3m，设 0.03~0.05m 合适）
FINAL_LS_POSE_AXIS_SIZE = max(RASTER_STEP * 0.50, 0.03)
...
# =========================================================
# 2.1 手眼标定 YAML 读取 + cam0 → base 转换
# =========================================================

T_BASE_CAM0_CACHE = None


def _find_calibration_matrix_recursive(obj):
    """
    在 YAML 解析结果中递归查找 T_base_cam0 或 T_base_to_cam。
    兼容：
      result:
        T_base_cam0: ...
      calibration:
    ...
    if isinstance(obj, dict):
        for key in ("T_base_cam0", "T_base_to_cam"):
            if key in obj:
                return obj[key]

        for value in obj.values():
            found = _find_calibration_matrix_recursive(value)
...
def _matrix4x4_from_any(value):
    arr = np.asarray(value, dtype=np.float64)
    if arr.shape == (4, 4):
        return arr
    if arr.size == 16:
        return arr.reshape(4, 4)
    raise ValueError(f"标定矩阵不是 4x4，当前 shape={arr.shape}, size={arr.size}")


def parse_yaml_calibration(filepath):
    """
    读取 eye_to_hand_result.yaml 中的 T_base_cam0 / T_base_to_cam。

    返回：
        4x4 numpy 矩阵，含义为 p_base = T_base_cam0 @ p_cam0。

    注意：
        YAML 内矩阵平移量单位应为米；本程序 PLY 轨迹点也按米处理。
    """
    if not os.path.exists(filepath):
        raise FileNotFoundError(f"标定 YAML 文件不存在：{filepath}")

    # 优先使用 PyYAML；没有 PyYAML 时自动回退到纯文本解析。
    try:
        import yaml  # type: ignore
        with open(filepath, "r", encoding="utf-8") as f:
            data = yaml.safe_load(f)
        value = _find_calibration_matrix_recursive(data)
        if value is not None:
            return _matrix4x4_from_any(value)
    except Exception:
        pass
    ...
    key_idx = -1
    key_re = re.compile(r"^\s*(T_base_cam0|T_base_to_cam)\s*:")
    ...
    # 同一行可能存在内联矩阵，例如 T_base_cam0: [[...]]
    after_colon = lines[key_idx].split(":", 1)[1]
    values.extend(float(x) for x in num_re.findall(after_colon))
    ...
    for line in lines[key_idx + 1:]:
        ...
        # 矩阵块通常以 '-'、'[' 或数字开头；遇到下一个 key 则停止。
        if re.match(r"^[A-Za-z_][\w\-]*\s*:", stripped) and len(values) > 0:
            break

        if stripped.startswith("-") or stripped.startswith("[") or re.match(r"^[-+]?\d", stripped):
            values.extend(float(x) for x in num_re.findall(stripped))
        elif len(values) > 0:
            break
    ...
    if len(values) < 16:
        raise ValueError(f"解析 T_base_cam0 失败，只找到 {len(values)} 个数值。")
...
def get_T_base_cam0():
    """
    获取并缓存 T_base_cam0。若关闭转换，则返回单位矩阵。
    """
    global T_BASE_CAM0_CACHE

    if not ENABLE_CAM0_TO_BASE_TRANSFORM_FOR_LS:
        return np.eye(4, dtype=np.float64)

    if T_BASE_CAM0_CACHE is None:
        T_BASE_CAM0_CACHE = parse_yaml_calibration(YAML_PATH)

    return T_BASE_CAM0_CACHE


def transform_points_normals_cam0_to_base(points_m, normals=None):
    """
    将 cam0 坐标系下的点和法向转换到机器人 base 坐标系。
    ...
    pts_base = (R_bc @ pts.T).T + t_bc.reshape(1, 3)

    if normals is None:
        return pts_base, None

    nms = np.asarray(normals, dtype=np.float64)
    if nms.ndim == 1:
        nms = nms.reshape(1, 3)

    nms_base = (R_bc @ nms.T).T
    n_len = np.linalg.norm(nms_base, axis=1, keepdims=True)
    n_len[n_len < 1e-12] = 1.0
    nms_base = nms_base / n_len

    return pts_base, nms_base


def print_cam0_to_base_transform_summary():
    """
    打印最终 LS 坐标转换信息，方便确认 YAML 是否加载正确。
    """
    if not ENABLE_CAM0_TO_BASE_TRANSFORM_FOR_LS:
        print("[CALIB] ENABLE_CAM0_TO_BASE_TRANSFORM_FOR_LS=False，最终 LS 不做 cam0→base 转换。")
        return

    T = get_T_base_cam0()
    R_bc = T[:3, :3]
    t_bc_m = T[:3, 3]
    t_bc_mm = t_bc_m * 1000.0

    print("\n================ cam0 → base 标定转换 ================")
    print("[CALIB] YAML_PATH:", YAML_PATH)
    print("[CALIB] coordinate_usage: p_base = T_base_cam0 @ p_cam0")
    print("[CALIB] R_base_cam0:")
    for row in R_bc:
        print("        [" + ", ".join(f"{v: .8f}" for v in row) + "]")
    print("[CALIB] t_base_cam0 (m) :", [float(f"{v:.8f}") for v in t_bc_m])
    print("[CALIB] t_base_cam0 (mm):", [float(f"{v:.3f}") for v in t_bc_mm])
    print("=====================================================\n")
...
def make_ls_pose_sequence_from_face_records(face_records, face_name):
    ...
    关键变化：
    1. face_records 中的 x/y/z 和 nx/ny/nz 默认来自 cam0 坐标系；
    2. 生成最终 LS 位姿前，先将 XYZ 和法向一起转换到机器人 base 坐标系；
    3. WPR 使用转换后的 base 坐标轨迹切线 + base 坐标表面法向重新计算；
       因此工具轴仍保持垂直于表面，而不是简单沿用 cam0 姿态角；
    4. HOME 点默认不写入，避免 0,0,0 或错误 HOME 导致位置/形态不一致。
    """
    ...
    surface_points_cam0 = rec[:, 0:3]
    normals_cam0 = rec[:, 3:6]
    segment_ids = rec[:, 12].astype(np.int32)

    # 法向归一化；随后如果启用标定转换，会再用 R_base_cam0 旋转到 base。
    n_len = np.linalg.norm(normals_cam0, axis=1, keepdims=True)
    n_len[n_len < 1e-12] = 1.0
    normals_cam0 = normals_cam0 / n_len

    # cam0 → base：点使用 R+t，法向只使用 R。
    if ENABLE_CAM0_TO_BASE_TRANSFORM_FOR_LS:
        surface_points, normals = transform_points_normals_cam0_to_base(
            surface_points_cam0,
            normals_cam0
        )
    else:
        surface_points = surface_points_cam0
        normals = normals_cam0
    ...
    for seg_i, seg_id in enumerate(unique_segments):
        ...
        # TCP 执行点。默认 SURFACE_STANDOFF_DISTANCE_MM=0，即使用表面点；
        # 如果需要离开表面，可按“米”设置该值，例如 0.10 表示 100mm。
        spray_points = seg_points_surface + seg_normals * float(SURFACE_STANDOFF_DISTANCE_MM)

        # WPR 根据 base 坐标系下的轨迹切线 + 表面法向计算，保持工具垂直表面。
        ...
        if LS_USE_HOME_POINTS and LS_RETRACT_BETWEEN_SEGMENTS and seg_i < len(unique_segments) - 1:
            add_pose(home, "J", float(LS_HOME_JOINT_SPEED_PERCENT), "FINE", f"{face_name}_SEG{seg_id}_HOME")

    if LS_USE_HOME_POINTS and LS_RETURN_HOME_AFTER_EACH_FACE:
...
"""
   ...
   GP1:
    ...
    X = {_fmt_ls_num(x_mm)} mm,    Y = {_fmt_ls_num(y_mm)} mm,    Z = {_fmt_ls_num(z_mm)} mm,
...
def save_ls_program(local_path, program_name, pose_sequence):
    ...
    if LS_STRICT_PAINT_HEADER and b"/APPL" not in raw:
...
def export_face_ls_files(trajectory_result, face_id_to_name, output_dir):
    ...
    根据每个面的轨迹生成：
    1. 每个面一个 LS 文件；
    2. 可选生成一个所有面的 combined LS 文件。

    每个面结束后：
        末端沿该面法向后退到安全空间，再回 HOME。
    """
    ...
    for fid in sorted(per_face_records.keys()):
        ...
        if LS_GENERATE_COMBINED_PROGRAM:
            # 合并程序保留每个面自己的 HOME -> APPROACH -> SPRAY -> RETREAT -> HOME 逻辑。
            if len(combined_pose_rows) == 0:
                ...
                # 如果上一个面已经 HOME 结束，直接接下一个面的 pose_seq。
                combined_pose_rows.extend(pose_seq["poses"].tolist())
                combined_move_types.extend(pose_seq["move_types"])
                combined_speeds.extend(pose_seq["speeds"])
                combined_cnts.extend(pose_seq["cnts"])
    ...
    if LS_GENERATE_COMBINED_PROGRAM and len(combined_pose_rows) > 0:
        combined_seq = {
        ...
        # 汇总多个面的轨迹到一个固定文件名的 LS 文件。
        # 文件名按用户要求固定为 test20250910wk2.ls。
    ...
    if LS_GENERATE_COMBINED_PROGRAM:
        print("[LS] combined LS saved as:", os.path.join(LS_OUTPUT_DIR, LS_COMBINED_OUTPUT_FILENAME))
...
def show_open3d_window(geoms, window_name, point_size, lookat_center=None):
    ...
    if lookat_center is not None:
        ...
        # front = 相机指向物体的方向
        front = np.array([0.5, -0.5, -0.5], dtype=np.float64)
        front = front / np.linalg.norm(front)
        # set_lookat + set_front + set_up 是 Open3D 原生方法，会正确处理相机位置
        ctrl.set_lookat(np.array(lookat_center, dtype=np.float64))
...
def main():
    ...
    print("[PARAM] ENABLE_CAM0_TO_BASE_TRANSFORM_FOR_LS:", ENABLE_CAM0_TO_BASE_TRANSFORM_FOR_LS)
    print("[PARAM] YAML_PATH:", YAML_PATH)
    ...
    # 提前加载并打印标定矩阵，避免 LS 导出到最后才发现 YAML 路径或字段错误。
    print_cam0_to_base_transform_summary()
    ...
    coord_size = max(extent * 0.08, RASTER_STEP)
    coord = o3d.geometry.TriangleMesh.create_coordinate_frame(size=coord_size)
    print("\n[VIS] 显示原始 Mesh，关闭窗口后继续...")
    o3d.visualization.draw_geometries([mesh_vis, coord],
                                       window_name="1. 原始 Mesh",
    ...
    # === 可视化2: 原始扫描点 ===
    if len(raw_points) > 0:
        mesh_vis2 = copy.deepcopy(mesh)
        mesh_vis2.paint_uniform_color([0.72, 0.72, 0.72])
        mesh_vis2.compute_vertex_normals()
        pcd_raw = o3d.geometry.PointCloud()
        pcd_raw.points = o3d.utility.Vector3dVector(raw_points)
        if len(raw_normals) > 0:
            pcd_raw.normals = o3d.utility.Vector3dVector(raw_normals)
        pcd_raw.paint_uniform_color([1, 0, 0])  # 红色
        print("[VIS] 显示原始扫描点（红色），关闭窗口后继续...")
        o3d.visualization.draw_geometries([mesh_vis2, pcd_raw, coord],
                                           window_name="2. 原始扫描点（红色）",
                                           width=1024, height=768)
    else:
        print("[VIS][WARN] 原始扫描点为 0，跳过可视化")
        # 仍展示 mesh，让用户确认
        mesh_vis2 = copy.deepcopy(mesh)
        mesh_vis2.paint_uniform_color([0.72, 0.72, 0.72])
        mesh_vis2.compute_vertex_normals()
        print("[VIS] 扫描点为 0，显示 Mesh 供检查，关闭窗口后继续...")
        o3d.visualization.draw_geometries([mesh_vis2, coord],
                                           window_name="2. 扫描点为 0 — 检查 Mesh",
                                           width=1024, height=768)
    ...
    # === 可视化3: Step2 去重点 ===
    mesh_vis3 = copy.deepcopy(mesh)
    mesh_vis3.paint_uniform_color([0.72, 0.72, 0.72])
    mesh_vis3.compute_vertex_normals()
    if len(step2_points) > 0:
        pcd_step2 = create_point_cloud_by_face_color(
            points=step2_points, normals=step2_normals,
            face_ids=step2_face_ids, face_id_to_name=face_id_to_name
        )
        print("[VIS] 显示 Step2 去重点（按面着色），关闭窗口后继续...")
        o3d.visualization.draw_geometries([mesh_vis3, pcd_step2, coord],
                                           window_name="3. Step2 去重点（按面着色）",
                                           width=1024, height=768)
    else:
        print("[VIS][WARN] Step2 点为 0，跳过可视化")
    ...
    if ENABLE_FACE_TRAJECTORY_CONNECTION:
        ...
        # === 可视化4: 轨迹线 ===
        if len(trajectory_result["lines"]) > 0 and len(step2_points) > 0:
            mesh_vis4 = copy.deepcopy(mesh)
            mesh_vis4.paint_uniform_color([0.72, 0.72, 0.72])
            mesh_vis4.compute_vertex_normals()
            ordered_indices = trajectory_result["ordered_indices"]
            ordered_pcd = o3d.geometry.PointCloud()
            ordered_pcd.points = o3d.utility.Vector3dVector(step2_points[ordered_indices])
            ordered_pcd.paint_uniform_color([1, 0, 0])  # 红色轨迹点
            # 轨迹线
            lines = trajectory_result["lines"]
            line_pts = step2_points
            line_set = o3d.geometry.LineSet()
            line_set.points = o3d.utility.Vector3dVector(line_pts)
            line_set.lines = o3d.utility.Vector2iVector(lines)
            line_set.paint_uniform_color([0, 0, 1])  # 蓝色轨迹线
            print("[VIS] 显示轨迹线（蓝色）+ 轨迹点（红色），关闭窗口后继续...")
            o3d.visualization.draw_geometries([mesh_vis4, ordered_pcd, line_set, coord],
                                               window_name="4. 轨迹线（蓝）+ 轨迹点（红）",
                                               width=1024, height=768)
        else:
            print("[VIS][WARN] 轨迹为空，跳过可视化")
    ...
    print("         ls_per_face/M10D12_EXEC.ls")
